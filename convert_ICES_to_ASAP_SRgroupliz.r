...
# Liz Brooks
# Version 1.0
# also uses : SAM read.ices fn modified by Dan Hennen 
##
...
#---------------------------------------------------------------------------
#---------------------------------------------------------------------------
# Function to set-up asap3 "west coast style" (indices with age comp)
# Liz Brooks
# Version 1.0
...
"setup.asap.w" <-function(wd, od, model.id, nyears, first.year, asap.nages, nfleets,
        ...
        fish.ind, sel.ind, ind.age1, ind.age2, ind.use, ind.sel.mats, ind.mat, ind.cv=index.cv, ind.neff=ind.neff,
        p.Fmult1, p.Fmult.dev, p.recr.dev, p.N1, p.q1, p.q.dev, p.SR, p.h, recr.CV=recr.CV, lam.ind,
        lam.c.wt, lam.disc, catch.CV=catch.cv, disc.CV, Neff.catch=Neff.catch, Neff.disc, lam.Fmult.y1,
        ...
        naa.y1, Fmult.y1, q.y1, SSB0, unexpPar, h.guess=h.guess, F.max, ignore.guess,
...
write( '# S-R Unexploited specification ( 0= SSB0  1= R0 )' , file=out.file,append=T)   #, ncol=(nyears))
write(unexpPar, file=out.file,append=T, ncol=1  )

write( '# Unexploited initial guess' , file=out.file,append=T)   #, ncol=(nyears))
write(SSB0, file=out.file,append=T, ncol= 1 )
...
tt <- round(tt*12, 0)
#LIZ edit (ASAP index for months is 1-13)
if (tt==0)  tt <- 1
...
#ICES2ASAP <- function(user.wd,user.od,model.id,ices.id)
  ICES2ASAP <- function(user.wd,user.od,model.id,ices.id="", h.guess, p.h, unexpPar, SSB0guess, recr.CV, catch.CV, Neff.catch, ind.cv, ind.neff, Frep.age, Fmult.y1, f.sel.type)
  { 
  cn <- read.ices(paste(user.wd,ices.id,"sim-LANUM.txt",sep=""))
  print(cn)
  cw <- read.ices(paste(user.wd,ices.id,"sim-WELAND.txt",sep=""))
  #dw <- read.ices(paste(user.wd,ices.id,"dw.dat",sep=""))
  mo <- read.ices(paste(user.wd,ices.id,"sim-MATPROP.txt",sep=""))
  nm <- read.ices(paste(user.wd,ices.id,"sim-NATMOR.txt",sep=""))
  propf <- read.ices(paste(user.wd,ices.id,"sim-FPROP.txt",sep=""))
  pm <- read.ices(paste(user.wd,ices.id,"sim-MPROP.txt",sep=""))
  sw <- read.ices(paste(user.wd,ices.id,"sim-WEST.txt",sep=""))
  surveys <- read.ices(paste(user.wd,ices.id,"sim-TUNE.txt",sep=""))
  ...
  catch.yy <- scan(paste(user.wd,ices.id,"sim-LANUM.txt",sep=""), what=integer(),  sep="", skip=2, nlines=1 ) 
  ...
  catch.ages <- scan(paste(user.wd,ices.id,"sim-LANUM.txt",sep=""), what=integer(),  sep="", skip=3, nlines=1 )
  catch.ages = seq(catch.ages[1],catch.ages[2],1)
  asap.ages = 1:catch.nages
  # assuming 2 WAA matrices (catch, and spawning weight; assume no discard weights and just dummy pointer to catch weight); since assuming 1 fleet, cw should equal lw in ASAP)
  waa.array <- array(NA, dim=c(catch.nyrs, catch.nages, 2))
  waa.array[,,1] <- cw[1:catch.nyrs,]
  waa.array[,,2] <- sw[1:catch.nyrs,] #
  
  waa.pointer.vec <- c(1, 1, 1, 1, 2, 2) #assuming spawning weight-jan-1 biomass
  ...
  print("f.peak")
  print(f.peak)
  f.sel.mats.c1 <- c( seq(0.1,0.9, length.out=(catch.nages)),  round((f.peak)/2,2), 0.25,
  ...
  recr.CV <- rep(recr.CV, catch.nyrs)
  catch.CV <- rep(catch.CV, catch.nyrs)
  disc.CV <- rep(0, catch.nyrs)
  Neff.catch <- rep(Neff.catch, catch.nyrs)
  Neff.disc <- rep(0, catch.nyrs)
  #Fmult.y1 <- 0.1
  if (f.sel.type==1) tmp.f.sel <- f.sel.mats[1:catch.nages, 1]
  if (f.sel.type==2) tmp.f.sel <- logistic(x=catch.ages, a.50=f.sel.mats[(catch.nages+1), 1] , b=f.sel.mats[(catch.nages+2), 1])
  if (f.sel.type==3) tmp.f.sel <- db.logistic(x=catch.ages, a.50.up= f.sel.mats[(catch.nages+3), 1], b.up=f.sel.mats[(catch.nages+4), 1] , a.50.down= f.sel.mats[(catch.nages+5), 1], b.down=f.sel.mats[(catch.nages+6), 1] )
  
  naa.y1 <- ((nm[1,]+Fmult.y1*tmp.f.sel)/Fmult.y1*tmp.f.sel)*cn[1,]/(1-exp(-nm[1,]-Fmult.y1*tmp.f.sel))
  ...
  fleet.names <- "#$fleet1"
  survey.names <- paste0(rep("#$", n.surveys), names(surveys))
  fleet.dir <-  rep(1,nfleets)
  disc.flag =  F
  ...
  setup.asap.w(wd=user.wd, od=user.od, model.id=model.id, nyears=catch.nyrs,
               ...
               p.Fmult1=1, p.Fmult.dev=3, p.recr.dev=3, p.N1=2, p.q1=1, p.q.dev=-1, p.SR=1, p.h=p.h,
               recr.CV=recr.CV, lam.ind=rep(1,n.surveys),
               lam.c.wt=rep(1,nfleets), lam.disc=rep(0,nfleets), catch.CV=catch.CV, disc.CV=disc.CV,
               Neff.catch=Neff.catch, Neff.disc, lam.Fmult.y1=rep(0, nfleets),
               ...
               naa.y1, Fmult.y1=Fmult.y1, q.y1, SSB0=SSB0guess, unexpPar=unexpPar, h.guess=h.guess, F.max=5, ignore.guess=0,
